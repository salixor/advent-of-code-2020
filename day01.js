...
const doTheMagic = currentValues => entries.map(value => {
...
const listThreeProducts = entries.reduce(
  (acc1, curr1) => acc1.concat(
    ...entries.reduce((acc2, curr2) =>
      (curr1 + curr2 < SUM_2020) ? acc2.concat(...doTheMagic([curr1, curr2])) : acc2, [])
  ), []
)
